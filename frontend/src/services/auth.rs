...
/// Get  user info
pub async fn get(username: String) -> Result<UserInfoWrapper, Error> {
    request_get::<UserInfoWrapper>(format!("/user/{}", username)).await
}
...
pub async fn save(user_update_info: UserUpdateInfoWrapper) -> Result<UserInfoWrapper, Error> {
    request_put::<UserUpdateInfoWrapper, UserInfoWrapper>("/users".to_string(), user_update_info)
        .await
}
